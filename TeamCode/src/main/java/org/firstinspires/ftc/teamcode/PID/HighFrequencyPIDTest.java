...
public class HighFrequencyPIDTest extends LinearOpMode {
	...
	public void runOpMode() throws InterruptedException {
		...
		while (!isStopRequested() && opModeIsActive()) {
			...
			double motorVelo = s1.getVelocity();
			Log.i("velo 1", Double.toString(motorVelo));
			Log.i("pos 1", Double.toString(s1.getCurrentPosition()));

			double pow = veloController.update(s1.getCurrentPosition(), motorVelo);
